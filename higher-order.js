...
//Code Here
let evenNumbers = mixedNumbers.filter((curVal) => curVal % 2 === 0);
...
//Code Here
let postTaxPrices  = prices.map((curVal) => curVal * 1.07);
...
//Code Here
let totalPopulation   = populations.reduce((total, curVal) => total + curVal)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((curVal) => curVal.CP > 200);
...
let orderTotals = orders.map((curVal) => curVal.price * (curVal.tax + 1))
...
let bobsTotal = purchases.reduce((total, curVal) =>
{
  if(curVal.owner === "Bob")
  {
